...
public class AndroidMidiParser {
    Note[] notes;
    ArrayList<Note> noteArrayList = new ArrayList<Note>();
    Context context;

    AndroidMidiParser(Context context) {
    ...
    public Note[] parse(String file) {
        ...
        try {
            ...
            if (file.contains("inappmidi/")) {
                return new Note[]{new Note(0, 0, 0, 0, 0)};
                //inputStream = fileInputStream;//Gdx.files.internal(file).read();
            } else {
                inputStream = new FileInputStream(new File(file));
            }

            int chunkMagic = readFixedLengthCode(inputStream, 4);
            System.out.println("ChunkMagic: " + chunkMagic);
